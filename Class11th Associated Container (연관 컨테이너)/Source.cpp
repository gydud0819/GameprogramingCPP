...
#pragma region 레드 블랙 트리
	/*
	* 레드 : 삽입
	* 새로 들어가는 노드 값은 항상 레드이다.
	* 블랙 : 삭제
	* 루트는 항상 블랙이어야 한다.
	* 부모 노드와 자식 노드 둘다 레드가 되면 안된다?
	*
	* <Double Red>
	* 삼촌 노드가 검은색이면 -> Restructuring
	*
	* 삼촌 노드가 빨간색이면 -> Recoloring
	*
	*
	*
	*
...
#pragma region set
	...
	//
	//for (const int& elem : s)		// 웬만해선 const를 붙이는게 좋다
	//{
	//	cout << elem << "  ";
	//}
...
#pragma region map
	...
	m.insert({ "최고 기온", 34.5f });
	m.insert({ "체감 온도", 41.2f });
